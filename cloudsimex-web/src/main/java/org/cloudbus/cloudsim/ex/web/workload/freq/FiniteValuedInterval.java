...
public class FiniteValuedInterval {
    ...
    private static boolean validarMatcher(Matcher matcher, int index, String caracter) {
    	if (matcher.group(index) != null) {
    		return caracter.equals(matcher.group(index));
    	}
    	return false;
    }
    
    private static double validarMatcher(Matcher matcher, int index) {
    	if (matcher.group(index) != null) {
    ...
    private static FiniteValuedInterval createInterval(String interval, byte[] seed, SeedGenerator seedGen) {
        ...
        matcher.matches();
        boolean includeStart = validarMatcher(matcher, 1, "[");
        double start = validarMatcher(matcher, 2);
        double end = validarMatcher(matcher, 3);
        boolean includeEnd = validarMatcher(matcher, 1, "]");

        double mean = validarMatcher(matcher, 5);
        double stDev = validarMatcher(matcher, 6);
